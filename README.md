...
## Brief description
A CLI program written mostly in python that generates a daily log to record what one did that day in terms of coding work. It asks the user a series of questions about their coding activities that day and writes the answers to a markdown file. At first, it will be trigged directly by the user in a CLI, so it will be up to the user to decide when to generate a log. 

See the [[functionality overview]](https://github.com/rhytidia/coding_log_python/wiki/Functionality-overview) page on the project wiki for more details.
